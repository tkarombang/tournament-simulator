...
export default function KnockoutStage() {
  ...
  // SIMPAN SKOR KE LOCALSTORAGE setiap kali knockoutScores berubah
            ...
            return (
              <div key={index} className="flex flex-col items-center bg-gray-50 dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-md border-gray-200 dark:border-gray-700">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-2 text-gray-800 dark:text-gray-200">
                  ...
                  <div className="flex flex-col items-center gap-1 w-full sm:w-40">
                    ...
                    <div className="flex items-center gap-1">
                      <button className="bg-gray-100 dark:bg-gray-700 border border-gray-300 rounded-l-sm flex items-center p-1 h-5 w-5 text-gray-600 dark:text-gray-400 text-xs">-</button>
                      <input type="text" value="0" readOnly className="bg-gray-50 border-x-0 border-gray-300 text-center w-8 py-0.5 dark:bg-gray-700 dark:border-gray-600 text-xs" />
                      <button className="bg-gray-100 dark:bg-gray-700 border border-gray-300 rounded-r-sm flex items-center p-1 h-5 w-5 text-gray-600 dark:text-gray-400 text-xs">+</button>
                    </div>
                  </div>
                ...
                <div className="mt-2 flex justify-center">
                  <button className="px-3 py-1 rounded text-xs text-white bg-blue-500 dark:bg-blue-700 transition-colors">Simulasi</button>
                </div>
              </div>
            );
          })}
        ...
        <div className="flex justify-center md:w-30 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex flex-wrap gap-2 justify-center md:grid md:grid-cols-2 lg:grid lg:grid-cols-1">
            {activeTab === "Round Of 16"
              ? groups.map((group) => (
                  <button key={group} className=" px-2 py-1 rounded text-xs text-white bg-red-500 dark:bg-red-700">
                    Reset {group}
                  </button>
                ))
              : matches[activeTab].map((_, index) => {
                  ...
                  return (
                    <button key={label} className=" px-2 py-1 rounded text-xs text-white bg-red-500 dark:bg-red-700">
                      {label}
                    </button>
                  );
                })}
